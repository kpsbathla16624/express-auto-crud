...
import type { AutoCrudOptions, ListResponse } from './types.js';
...
export function autoCRUD(
  ...
  const buildFilter = (query: any): any => {
    ...
    if (!opts.filter.enabled) return filter;
  ...
  // Helper: Apply sort
  const applySort = (query: any): string => {
    if (query.sort) {
  ...
  const getMiddlewares = (specific?: any[]) => {
  ...
  app.get(
    baseRoute,
    ...getMiddlewares(opts.middleware.list),
    async (req: Request, res: Response) => {
      try {
        const page = Math.max(1, parseInt(req.query.page as string) || 1);
        const limit = Math.min(
          opts.pagination.maxLimit,
          parseInt(req.query.limit as string) || opts.pagination.defaultLimit
        );
  ...
  app.get(
    `${baseRoute}/:id`,
    ...getMiddlewares(opts.middleware.getOne),
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
  ...
  app.post(
    baseRoute,
    ...getMiddlewares(opts.middleware.create),
    async (req: Request, res: Response) => {
      try {
        let data = req.body;
  ...
  app.put(
    `${baseRoute}/:id`,
    ...getMiddlewares(opts.middleware.update),
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
        let data = req.body;
  ...
  app.delete(
    `${baseRoute}/:id`,
    ...getMiddlewares(opts.middleware.delete),
    async (req: Request, res: Response) => {
      try {
        const { id } = req.params;
